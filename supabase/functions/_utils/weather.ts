...
export function getFullCountryName(countryCode) {
  return countryAbbreviations[countryCode] || countryCode;
}
...
export function formatSearchTerm(searchTerm) {
  ...
  if (/^\d{5}$/.test(searchTerm.slice(0, 5))) {
    return `${searchTerm},US`;
  }

  // Split the search term into components
  const components = searchTerm.split(',').map(comp => comp.trim());
  ...
  const city = components[0];
  let state = components.length > 1 ? components[1] : null;
  const country = components.length > 2 ? components[2] : null;
  ...
  const isUSState = state && Object.values(stateAbbreviations).includes(state);
  const stateText = state ? `,${state}` : '';
  const countryText = country ? `,${country}` : isUSState ? ',US' : '';

  const formattedSearch = `${city}${stateText}${countryText}`;
